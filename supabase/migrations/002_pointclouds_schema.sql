...
CREATE TABLE pointclouds.PointClouds (
    ...
    CONSTRAINT chk_processing_status_variants CHECK (
        (VariantTypeID = (SELECT VariantTypeID FROM shared.VariantTypes WHERE VariantTypeName = 'original') AND ProcessingStatus IS NULL)
        OR
        (VariantTypeID != (SELECT VariantTypeID FROM shared.VariantTypes WHERE VariantTypeName = 'original'))
    ),
