...
CREATE TABLE shared.ClimateZones (
    ...
    Description TEXT,
    CONSTRAINT chk_climate_zone_format CHECK (ClimateZoneName ~ '^[A-Z][a-z]{0,2}[a-z]?$')
);

COMMENT ON TABLE shared.ClimateZones IS 'Köppen climate classification zones';
COMMENT ON COLUMN shared.ClimateZones.ClimateZoneName IS 'Köppen climate classification code (e.g., Cfb, Dfb)';
