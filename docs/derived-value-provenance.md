...
`shared.attribute_provenance` precedent this follows)
...
`shared.attribute_provenance` (`location_id`, `column_name`, `process_id`,
...
each meaningful for exactly one other column — the same design `shared.attribute_provenance`
...
* New `shared.attribute_provenance` table, plus a write helper mirroring
...
`shared.attribute_provenance`. It is the one derived value that already has a working
...
* XRFF-369 / migration `20260902140000_open_data_landing_zones.sql` — the
  `shared.attribute_provenance` precedent.
